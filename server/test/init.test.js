...
module.exports.init = async () => {
  ...
  bulkCreateTest().then(() => console.log("bulkCreateTest done")).catch(err => console.log(err));
...
const periods = await Period.bulkCreate(
  Array.from({ length: periodCount }, (_, index) => ({
    ...
    start: (new Date().setMinutes(index*10)),
    durationMinutes: 30
  }))
...
await Promise.all(admins.map((admin, index) => {admin.addSalon(salons[index +1 % salonCount])}))
...
await Promise.all(salons.map((salon, index) => salon.addMaster(masters[index % masterCount])))
...
// await Promise.all(masters.map((master, index) => master.addClientcards(clientcards[index % salonCount])))
db.sequelize.sync();
let fetchedCards= [];
